...
class Stopwatch
{
    public:
        ...
        void clear() {_extra=0.0; _running=false;};
        /** Reset stopwatch to zero and start. */
        void restart() {_extra=0.0; _start=now(); _running=true;};
        /** Get current reading on stopwatch. */
        double getElapsed() const {return (_running?now()-_start:0)+_extra;};
        /** Stop the stopwatch. Retain time reading. */
        void stop() {if (_running) _extra+=now()-_start; _running=false;};
        /** Start the stopwatch. Retain time reading. */
        void start() {if (!_running) {_start=now(); _running=true;};};
...
/** A stopwatch that measures in CPU time (ignoring time spent by system or other processes) */
