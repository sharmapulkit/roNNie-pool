...
void LogWriter::write(const GameState &gs) { _os << "STATE " << gs << endl; }

void LogWriter::writeShotParams(const ShotParams &sp) {
  _os << sp.a << " " << sp.b << " " << sp.theta << " " << sp.phi << " " << sp.v;
}

void LogWriter::write(const GameShot &gs, const ShotParams &noiselessParams,
                      double duration) {
  NoNoise nn;
  _os << "SHOT ";
  ...
  _os << " " << gs.ball << " " << gs.pocket << " " << gs.decision << " "
      << gs.cue_x << " " << gs.cue_y << " " << gs.timeSpent << " " << duration
      << " ";
  if (_noise)
    _noise->toStream(_os);
  else
...
ShotResult LogWriter::LogAndExecute(GameState &gst, const GameShot &gsh,
